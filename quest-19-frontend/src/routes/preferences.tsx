...
import { type UserPreferences, cookieUtils } from "@/lib/cookies"
  ...
  const addCustomCategory = () => {
    if (newCategory.trim() && !preferences.categories.includes(newCategory.trim())) {
      ...
      toast("Category added", {
        description: `"${newCategory.trim()}" has been added to your custom categories.`,
      })
    }
  }
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Customize your content experience by selecting your preferred news sources and creating custom categories.
            </p>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select the news sources you'd like to see content from. You can choose multiple sources.
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className='inline-flex justify-between w-full'>
                        ...
                        <img src={source.icon} alt="" className='size-6 rounded-full' />
                      </div>
              ...
              {preferences.sources.length > 0 && (
                <div className="mt-4 pt-4 border-t">
                  <p className="text-sm text-muted-foreground mb-2">Selected sources ({preferences.sources.length}):</p>
                  <div className="flex flex-wrap gap-2">
                    {preferences.sources.map((sourceId) => {
                      const source = DEFAULT_NEWS_SOURCES.find((s) => s.id === sourceId)
                      return source ? (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select from predefined categories or create your own custom categories to personalize your content.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <Label className="text-base font-semibold">Predefined Categories</Label>
                    ...
                    {PREDEFINED_CATEGORIES.map((category) => (
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={() => handlePredefinedCategoryToggle(category.id)}
                          className="mt-1"
                        />
                        <div className="flex-1 min-w-0">
                          <Label htmlFor={`predefined-${category.id}`} className="text-sm font-medium cursor-pointer">
                            {category.name}
                          </Label>
                  ...
                  {preferences.categories.filter((cat) => isPredefinedCategory(cat)).length > 0 && (
                    <div className="mt-4 pt-4 border-t">
                      <p className="text-sm text-muted-foreground mb-2">
                        Selected predefined categories (
                        {preferences.categories.filter((cat) => isPredefinedCategory(cat)).length}):
                      </p>
                      <div className="flex flex-wrap gap-2">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <Label className="text-base font-semibold">Custom Categories</Label>
                    <Badge variant="outline" className="text-xs">
                      Your Own
                    </Badge>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={!newCategory.trim() || preferences.categories.includes(newCategory.trim())}
                      size="icon"
                    >
                  ...
                  {preferences.categories.filter((cat) => !isPredefinedCategory(cat)).length > 0 ? (
                    ...
                    <div className="text-center py-6 text-muted-foreground border-2 border-dashed rounded-lg">
                      ...
                      <p className="text-xs mt-1">Add your first custom category above to get started.</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Overview of your current content preferences.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {preferences.categories.filter((cat) => isPredefinedCategory(cat)).length > 0
                      ? `${preferences.categories.filter((cat) => isPredefinedCategory(cat)).length} predefined categor${preferences.categories.filter((cat) => isPredefinedCategory(cat)).length !== 1 ? "ies" : "y"} selected`
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {preferences.categories.filter((cat) => !isPredefinedCategory(cat)).length > 0
                      ? `${preferences.categories.filter((cat) => !isPredefinedCategory(cat)).length} custom categor${preferences.categories.filter((cat) => !isPredefinedCategory(cat)).length !== 1 ? "ies" : "y"} created`
                ...
                <div className="pt-2 border-t">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {preferences.categories.length > 0
                      ? `${preferences.categories.length} total categor${preferences.categories.length !== 1 ? "ies" : "y"} selected`
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            <Button variant="outline" onClick={resetPreferences} className="flex-1 sm:flex-none">
